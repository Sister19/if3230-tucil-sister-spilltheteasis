    ...
    if (k < size && l < size) {
        ...
        for (int m = 0; m < size/2; m++) {
            for (int n = 0; n < size/2; n++) {
                ...
                element                         = cuCadd(element, cuCmul(el_even_even, exponent));
                element                         = cuCadd(element, cuCmul(el_even_odd, cuCmul(var_even_odd, exponent)));
                element                         = cuCadd(element, cuCmul(el_odd_even, cuCmul(var_odd_even, exponent)));
                element                         = cuCadd(element, cuCmul(el_odd_odd, cuCmul(var_odd_odd, exponent)));
            }
        } 
